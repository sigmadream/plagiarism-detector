...
def bundle_codenet(args: argparse.Namespace) -> None:
    ...
    problem_directories = sorted(
        ...
        if path.is_dir() and re.fullmatch(r"p[0-9]{5}", path.name)
    )
    if not problem_directories:
        raise SystemExit(f"source root contains no pNNNNN directories: {source_root}")
    ...
    try:
        connection.execute("SET temp_directory = ?", [temporary_directory.as_posix()])
        batch_size = 100
        for start in range(0, len(problem_directories), batch_size):
            ...
            print(
                f"Bundled {start + len(batch)}/{len(problem_directories)} CodeNet problems",
                flush=True,
            )
        invalid = connection.execute(
            """
            SELECT count(*) FROM submissions
            WHERE problem_id = '' OR submission_id = ''
            """
        ).fetchone()[0]
        if invalid:
            raise RuntimeError(f"failed to parse {invalid} CodeNet source identities")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run comparable cppTR benchmarks on Project CodeNet and POJ-104."
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    codenet_parser = subparsers.add_parser(
        "bundle-codenet", help="Bundle a Project CodeNet C++ corpus."
    )
    codenet_parser.add_argument("source_root", type=Path)
    codenet_parser.add_argument("database", type=Path)
    codenet_parser.add_argument("--replace", action="store_true")
    codenet_parser.set_defaults(handler=bundle_codenet)
